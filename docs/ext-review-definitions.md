...
The connection sends the existing binary `MSG_STDOUT` frames containing ANSI screen snapshots, suitable for the existing xterm rendering infrastructure. Snapshots follow the approved window’s visible layout at its external positions and dimensions: all panes when unzoomed, only the active pane when zoomed. Screens are polled every 250 ms and sent when changed. Snapshots with mid-capture layout changes are discarded and retried on the next poll. This is current-screen observation, not a lossless output/history stream. The viewer should have enough local rows and columns to display the external window; resizing the viewer never resizes tmux.
